  ...
  <base-modal ref='baseModal'>
    <div class="w-full max-w-2xl bg-main-300 shadow-md rounded px-6 py-6">
      ...
      <div class="flex flex-col items-center w-full">
        ...
        <form method="post" enctype='multipart/form-data' class="flex flex-col justify-between items-center bg-main-300 w-11/12 sm:w-5/6 mx-auto rounded-lg">
          <div class="flex flex-row h-40 justify-between justify-items-center w-full">
            <div class="flex flex-col justify-between w-1/2">
              ...
              <!-- 비밀번호 -->
              <div class="bg-tc-500 p-0 rounded-sm flex flex-row items-center border-tc-300 border-1">
                <span class="p-2">
                ...
                <input v-model="state.password" class="m-0 p-0 w-full text-tc-200 border-none border-transparent focus:border-transparent focus:ring-0" type="password" placeholder="비밀번호 설정">
              </div>
            </div>
            <!-- 썸네일 -->
            <div class="w-1/2 h-40 ml-5 border-tc-300 border-1">
              <div class="flex items-center justify-center w-full h-full">
                <label class="flex flex-col justify-center items-center cursor-pointer rounded-sm w-full h-full bg-tc-500 hover:bg-gray-300">
                  ...
                  <p>{{ state.fileName }}</p>
                  <input id="thumbnail" type="file" class="opacity-0 h-0" @change="onUploadImage()" />
                </label>
              </div>
            </div>
          </div>
...
export default {
  ...
  setup () {
    ...
    //   })
    // }
    return { state, open, close, createRoom, baseModal, onUploadImage }
  }
}
</script>
